...
;; TIPO MÃO
;; Uma mão é
;; 1. lista vazia: (empty), ou
;; 2. uma lista de cartas: (list (make-carta ...) (make-carta ...))
...
(check-expect  (escova? CARTA-OUROS MESA-1) false)
(check-expect  (escova? CARTA-OUROS MESA-2) true)
;; =============================================================
;; QUESTÃO 4
;; =============================================================
;; jogada-escova: Mão Mesa -> String
;; Objetivo: Dado uma mão e uma mesa se houver possibilidade de escova, devolve a
;;           mensagem (string) "Escova!" caso contrário devolve a mensagem "Não faço escova."
;; Exemplos:
...
(check-expect  (jogada-escova MÃO-3 MESA-1) "Não faço escova.")
(check-expect   (jogada-escova MÃO-3 MESA-2) "Escova!")
...
(check-expect  (verifica-escova MÃO-3 MESA-2) (make-carta "ouros" 6))
(check-expect   (verifica-escova (list (make-carta "copas" 2) (make-carta "paus" 6) (make-carta "ouros" 6)) MESA-2) (make-carta "paus" 6))
...
(define (verifica-carta-nula mão)
  (cond
    ;; verifica se a primeira carta da mão não é nula, então retorna a carta
...
(check-expect  (verifica-carta-nula MÃO-3) (make-carta "copas" 2))
(check-expect  (verifica-carta-nula MÃO-2) (make-carta "paus" 6))
